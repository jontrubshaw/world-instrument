...
describe('stream source registry', () => {
  ...
  it('checks source compatibility against score metadata', () => {
    ...
    expect(
      streamSourceRegistry
        .compatibleSourcesForScore(sensorScoreMetadata)
        .map((source) => source.id),
    ).toEqual([BROWSER_SENSOR_STREAM_SOURCE_ID]);
    expect(
  ...
  it('normalizes browser sensor payloads through the shared stream state contract', async () => {
    ...
    expect(result.state).toMatchObject({
      ...
      receivedAt: '2026-06-15T12:05:00.100Z',
      sequence: 3,
      metadata: {
    ...
    expect(result.state.samples.find((sample) => sample.key === 'fallbackActive')).toMatchObject({
      kind: 'boolean',
      value: true,
    });
