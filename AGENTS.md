...
- Active implementation branch: `cursor/playwright-chromium-setup-8fd0` for `JTW-44`.
...
- `JTW-44`: Update Cursor Cloud environment to install Playwright Chromium. In Progress on branch `cursor/playwright-chromium-setup-8fd0`.
...
Cursor Cloud environment setup work is active in `JTW-44` on branch `cursor/playwright-chromium-setup-8fd0`. This is tooling/environment work for Playwright Chromium rather than product runtime work.
...
- Node 24 is required (`.npmrc` sets `engine-strict=true`, so npm refuses to run under the wrong major). It is installed via `nvm`; the checked-in Cursor Cloud install hook `.cursor/install.sh` selects Node 24 before running `npm install`.
...
- Cursor Cloud uses `.cursor/environment.json`, whose blocking install command runs `.cursor/install.sh`; that script runs `npm install` and `npm run setup:browsers:with-deps` so Playwright Chromium is ready before agents run `npm run test:smoke`.
- The instrument app shell runs via Vite in `apps/instrument` with `npm run dev -w @world-instrument/instrument`.
- `npm run check` runs typecheck + lint + format:check + Vitest. Browser smoke coverage runs separately with `npm run test:smoke`, which starts the Vite app through Playwright. If Chromium ever goes missing in an existing agent session, recover under Node 24 with `npm run setup:browsers:with-deps`.
