...
- Active implementation issue: `JTW-52` is underway on branch `cursor/browser-sensor-source-d83f`.
...
- `JTW-52`: Add browser sensor stream source and route it through the instrument. In Progress on branch `cursor/browser-sensor-source-d83f`.
...
`JTW-52` is the active Cursor-delegated implementation issue. Scope is a registry-backed browser sensor/interaction source that uses pointer movement as the reliable baseline and DeviceMotion/DeviceOrientation only when supported and permissioned.
...
- PR #49 (`JTW-53 expose registry-backed source selection`) merged before `JTW-52` started; source selection now routes through `apps/instrument/src/sourceRuntime.ts`.
- JTW-52 implementation convention: `sensor.browser-interaction` is the app-facing sensor source. Pointer movement is always the live baseline, DeviceMotion/DeviceOrientation are opportunistic and permission-aware, and replay-safe capability/fallback metadata must be preserved in normalized stream state.
