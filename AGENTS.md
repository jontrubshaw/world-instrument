...
- Repo-level Cursor Cloud setup lives in `.cursor/environment.json`. Keep Playwright browser setup in `.cursor/install.sh` after `npm install` so `npx playwright install --with-deps chromium` uses the workspace's pinned Playwright package under Node 24.
